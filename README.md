...
- Support routing and register middleware in controller definition context using decorators.
- Auto binding http context(such as request, response and next objects) when a http request arrive to each controller object.
...
A controller class which can be assigned to a route must extends (instance of) BaseController class. Then, the controller object will have the http's context when a request arrive.
...
_note:_ the `@Route` decorator represents the Router generated by Express. Therefore, we could access to another standard **method** of the Express's router.


***RESTRICTION*** DO NOT miss the `@routingContext()` in each class definition when routing, it will cause unexpected on routing.
...
When making request to the enpoint
...
It happened because the we just set routing context for the *Controller* class, then the `@Route.get('/index')` decorator on *SecondController* class refers to the current routing context (*Controller*) so that the route will be mapped with `Controller.index` instead of `SecondController.index`.

In case if the `Controller` class did not define the `index` method, requesting to `GET /index` would cause ***Controller.index*** _is not a funtion_ error.
...
Using `@Middleware` like the follwing example is not be adviced, it will cause un expected on reading codes
...
*note: using this decorator means that we will calling the res.end(_content), so the response session will end there and all middlewares after this action could not interact with the response anything and anymore.
...
class Controller extends BaseControlle {
    ...
    doSomthing() {
        ...
        // "this.getUserInfo" here was transformed to type of DecoratorResult
        // DecoratorResult is the wrapper of the target method to cache metadatas of the 
        // decorator that take impact to the target method
        // Calling DecoratorResult.resolve() in order to resolve all the impacts that some decorators
