...
***RESTRICTION*** DO NOT miss the `@routingContext()` in each class definition when routing, it would cause unexpected on routing.
...
It happened because there is only routing context for *FirstController* class, then the `@Route.get('/index')` mapping on *SecondController* class refers to the current routing context (*FirstController*) so that the route will be mapped with `FirstController.index` instead of `SecondController.index`.
...
*note:* using this decorator means that we will calling the *res.send(_content)* and *res.end()*, so the response session will end there and all middlewares after this action could not interact with the response anything and anymore.
